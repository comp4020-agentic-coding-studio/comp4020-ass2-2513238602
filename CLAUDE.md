...
- Record patch/version context for roster data. Cache and optimise Riot's official static data with `pnpm assets:cache`; do not hotlink game art or depend on a live API for rendering.
...
- Layouts must remain readable at the two marking viewports, 1920×1080 and 390×844, without page scrolling or content clipped inside a component.
...
3. run the browser viewport contract across the marker's sampled pages, including keyboard focus and an interaction that survives resize;
4. run the course spec tests and `pnpm check:evidence` when process citations exist;
5. search tracked source for `STARTER_CONTENT`, placeholder copy, unchanged starter images, and accidental secrets;
6. review whether the change produced a clearer next action for a student.
